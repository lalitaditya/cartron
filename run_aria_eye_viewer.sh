...
su - "$REAL_USER" -c "cd '$SCRIPT_DIR' && export DISPLAY='$DISPLAY' && export WAYLAND_DISPLAY='$WAYLAND_DISPLAY' && export FASTRTPS_DEFAULT_PROFILES_FILE='$FASTRTPS_DEFAULT_PROFILES_FILE' && export FASTDDS_DEFAULT_PROFILES_FILE='$FASTDDS_DEFAULT_PROFILES_FILE' && source aria_env/bin/activate && python aria_teleop/aria_eye_viewer.py --display matplotlib --ephemeral-certs"
